...
import { core } from "../../../lib/data/team";
...
export default function Team(){
  ...
  return(
    <main className="bg-gra flex flex-col items-start justify-start md:justify-start md:items-start md:flex-row rounded-sm scrollbar-hide text-gray-100  bg-gray-900/5  snap-y snap-mandatory h-screen overflow-x-hidden  select-none">
      <HeadTag title="Core Members | AutoZ" cardTitle="core Members | AutoZ" description="US | AutoZ" image="" ></HeadTag>
         <Top main={"Core Members"} sub="AutoZ" mini={""}></Top>
     {/* <div className="w-full text-center font-Maven text-2xl">core MEMBERS | AUTOZ</div> */}
               ...
               {i.year && <div className="absolute top-0 right-auto w-full bg-gray   pr-1"><div className="text-gray-900 w-[95px] mx-auto bg-red500 ">{i.year} - {i.endYear}</div></div>}
                <div className="absolute top-0 pr-2 left-0 w-full h-full bg-black/20 flex flex-col items-end justify-end">
                  <div className="flex flex-row items-center justify-center space-x-1 scale-75">
                     <Social feature={i.feature} google={i.email} linkdin={i.linkdin} insta={i.insta}></Social>
                  </div>
...
function Social({feature,google,linkdin,insta}){
  ...
  return(
    <div className=' flex flex-row items-end justify-end space-x-2 opacity-80  text-gray-100 w-full '>
        ...
        {feature && <div>
          <RX initial={"90deg"} final={"0deg"} duration={0.6} delay={2.2}>
          <Link href={feature}>
          <a target="_blank" className='opacity-80 w-8 h-8 bg-gray-100 flex flex-row items-center justify-center rounded-full transition-all duration-300 ease-in-out hover:opacity-90 '>
            <svg className=' fill-black w-full pt-1 pl-1 mx-auto bg-gray-100 h-full rounded-full' width="24" height="24" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd"><path d="M16 3.383l-.924-.383-7.297 17.617.924.383 7.297-17.617zm.287 3.617l6.153 4.825-6.173 5.175.678.737 7.055-5.912-7.048-5.578-.665.753zm-8.478 0l-6.249 4.825 6.003 5.175-.679.737-6.884-5.912 7.144-5.578.665.753z"/></svg>
            </a>
          </Link>
          </RX>
        </div>
